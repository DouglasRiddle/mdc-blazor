...
window.MdcBlazor = {
    attachRipple: {
        init: function (elem) { mdc.ripple.MDCRipple.attachTo(elem); }
    },
    ...
    circularProgress: {
        init: function (elem) { mdc.circularProgress.MDCCircularProgress.attachTo(elem); }
    },
    ...
    dialog: {
        ...
        hide: function (elem) { if (elem._dialog) { elem._dialog.close(); } }
    },
    ...
    dialogWithCallback: {
        ...
        hide: function (elem, dialogAction) {
            if (elem._dialog) { elem._dialog.close(dialogAction || 'dismissed'); }
        }
    },
    ...
    focus: function (elem) {
        ...
        else { throw new Error('Unable to focus an invalid element.'); }
    },

    iconToggle: {
        init: function (elem) { mdc.iconButton.MDCIconButtonToggle.attachTo(elem); }
    },

    linearProgress: {
        init: function (elem) { mdc.linearProgress.MDCLinearProgress.attachTo(elem); }
    },
    ...
    list: {
        ...
        getSelectedIndex: function (elem) {
            if (elem._list) { return elem._list.selectedIndex; }
        },
        setSelectedIndex: function (elem, newIndex) {
            if (elem._list) { elem._list.selectedIndex = newIndex }
        },
    ...
    navDrawer: {
        ...
        init: function (drawerElement, listElement, mainContentElement, topAppBarElement) {
            ...
            const listItemListener = (e) => {
                ...
                if (!e.target.matches(".mdc-sub-list")) { drawer.open = false; }
            };
    ...
    radio: {
        init: function (elem, formElem, groupValue = "", radioValue = "") {
            ...
            if ((groupValue === radioValue) && (groupValue !== "" && radioValue !== "")) { radio.checked = true; }
        }
    },
    ...
    select: {
        ...
        setValue: function (elem, value) {
            if (elem._select) { elem._select.value = value; }
        },
        setDisabled: function (elem, value) {
            if (elem._select) { elem._select.disabled = value; }
        }
    },
    ...
    switch: {
        ...
        setChecked: function (elem, isSelected) {
            if (elem._switch) { elem._switch.selected = isSelected; }
        },
        setDisabled: function (elem, isDisabled) {
            if (elem._switch) { elem._switch.disabled = isDisabled; }
        }
    },
    ...
    textField: {
        ...
        layout: function (elem) {
            if (elem._textfield) { elem._textfield.layout(); }
        }
    },

    tooltip: {
        init: function (elem) { elem._tooltip = elem._tooltip || mdc.tooltip.MDCTooltip.attachTo(elem); }
    },

    topAppBar: {
        init: function (elem, scrollTarget) {
            const topAppBar = mdc.topAppBar.MDCTopAppBar.attachTo(elem);
            if (scrollTarget) { topAppBar.setScrollTarget(document.querySelector(scrollTarget)); }
        }
    }
};